...
const EditKidForm = ({ updateAgeContent, deleteAgeContent }) => {
  ...
  useEffect(() => {
    ...
    if (story && story.kids && story.kids.card) {
      const kid = story.kids.card.find(c => c.id === cardId);
      if (kid) {
        setFormData({
          title: kid.title || { en: '', te: '', hi: '' },
          description: kid.description || { en: '', te: '', hi: '' },
          timeToRead: kid.timeToRead || { en: '', te: '', hi: '' },
          storyType: kid.storyType || { en: '', te: '', hi: '' },
          thumbnailImage: kid.thumbnailImage || '',
          coverImage: kid.coverImage || '',
          partContent: kid.partContent || []
        });
        
        // Determine which languages have data
        const languagesWithData = [];
        if (kid.title?.en || kid.description?.en || kid.timeToRead?.en || kid.storyType?.en) {
          languagesWithData.push('en');
        }
        if (kid.title?.te || kid.description?.te || kid.timeToRead?.te || kid.storyType?.te) {
          languagesWithData.push('te');
        }
        if (kid.title?.hi || kid.description?.hi || kid.timeToRead?.hi || kid.storyType?.hi) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateAgeContent(storyId, cardId, 'kids', {
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this kids story?")) {
      try {
        await deleteAgeContent(storyId, cardId, 'kids');
        navigate('/stories/kids');
      } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Edit Kids Story</h1>
        <button
          onClick={() => navigate('/stories/kids')}
          className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-800"
        >
          <ArrowLeft size={18} /> Back to Kids Stories
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded shadow p-6 space-y-6">
        {/* Language Selection */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Languages for this Kids Story:</label>
